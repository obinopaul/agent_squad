...
class DaytonaInterpreter:
    ...
    def __init__(self, logger: logging.Logger, config: Config):
        ...
        self.workspace_root_dir: Optional[str] = None # Dynamically determined root directory
    ...
    async def _wait_for_command_exec_ready(self, max_wait_seconds: int = 60, interval_seconds: float = 2.0):
        """Poll the workspace until it is ready to accept commands."""
        self.logger.info(f"Waiting for workspace {self.workspace.id} command executor to become responsive...")
        start_time = time.time()
        while time.time() - start_time < max_wait_seconds:
            try:
                # Execute a simple, low-impact command to check for readiness
                response = self.workspace.process.exec("echo 'ready'")
                if response.exit_code == 0:
                    self.logger.info(f"Workspace {self.workspace.id} command executor is ready.")
                    return
            except Exception as e:
                self.logger.debug(f"Workspace command executor not ready yet, connection failed: {e}")
            
            await asyncio.sleep(interval_seconds)

        raise WorkspaceInitializationError(f"Workspace {self.workspace.id} command executor did not become responsive within {max_wait_seconds} seconds.")
    ...
    async def initialize_workspace(self) -> None:
        ...
        try:
            ...
            with FileLock(WORKSPACE_LOCK_FILE):
                ...
                workspace_id, created_at = await get_active_workspace()
                if workspace_id:
                    try:
                        ...
                        for workspace in workspaces:
                            if workspace.id == workspace_id:
                                ...
                                self.logger.info("Dynamically determining workspace root directory...")
                                response = self.workspace.process.exec("echo $HOME")
                                if response.exit_code == 0 and response.result:
                                    self.workspace_root_dir = response.result.strip()
                                    self.logger.info(f"Workspace root directory determined as: {self.workspace_root_dir}")
                                else:
                                    self.logger.warning("Could not determine workspace root directory dynamically, falling back to /home/workspace")
                                    self.workspace_root_dir = "/home/workspace"

                                # Initialize filesystem for this workspace. Even if it fails, we can proceed.
                                try:
                                    toolbox_api = ToolboxApi()
                                    self.filesystem = FileSystem(
                                        sandbox_id=self.workspace.id,
                                        toolbox_api=toolbox_api,
                                        get_root_dir=lambda: self.workspace_root_dir,
                                    )
                                    self.logger.info("FileSystem object initialized.")
                                except Exception as fs_err:
                                    self.logger.warning(f"Could not initialize FileSystem object: {fs_err}. File-based tools might fail, but command execution will work.")
                        ...
                        await clear_active_workspace(self.workspace)
                    ...
                    if len(workspaces) > 0:
                        ...
                        try:
                            ...
                            if workspaces:
                                ...
                                self.daytona.delete(oldest)  # Updated method name in newer SDK versions
                                self.logger.info(f"Removed workspace: {oldest.id}")
                        except Exception as e:
                ...
                while retry_count < max_retries:
                    try:
                        ...
                        # Wait for the command execution part of the workspace to be ready
                        await self._wait_for_command_exec_ready()

                        # Dynamically determine the root directory
                        self.logger.info("Dynamically determining new workspace root directory...")
                        response = self.workspace.process.exec("echo $HOME")
                        if response.exit_code == 0 and response.result:
                            self.workspace_root_dir = response.result.strip()
                            self.logger.info(f"Workspace root directory determined as: {self.workspace_root_dir}")
                        ...
                        # Initialize filesystem for this workspace. We will now proceed even if this fails.
                        try:
                            toolbox_api = ToolboxApi()
                            self.filesystem = FileSystem(
                                sandbox_id=self.workspace.id,
                        ...
                        await set_active_workspace(workspace_id, self.workspace)
    ...
    async def cleanup_workspace(self) -> None:
        ...
        try:
            ...
            with FileLock(WORKSPACE_LOCK_FILE):
                ...
                active_id, _ = await get_active_workspace(self.workspace)
                if active_id and active_id == workspace_id:
                    ...
                    while retry_count < max_retries and not success:
                        try:
                            ...
                            self.daytona.delete(self.workspace)  # Updated method name in newer SDK versions
                            success = True
            ...
            try:
                ...
                self.daytona.delete(self.workspace)
...
async def get_active_workspace(workspace: Optional[Workspace] = None):
    """
    Get the active workspace ID from the tracking file.
    Returns a tuple of (workspace_id, creation_time) or (None, None).
    
    Args:
        workspace: Optional Daytona Workspace instance for remote operations
    """
    logger = logging.getLogger("daytona-interpreter")
    
    # Try using Daytona's command execution if a workspace is provided
    if workspace:
        try:
            # Check if the file exists
            check_cmd = f"test -f {shlex.quote(WORKSPACE_TRACKING_FILE)}"
            response = workspace.process.exec(check_cmd)
            if response.exit_code == 0:
                # File exists, now read it
                cat_cmd = f"cat {shlex.quote(WORKSPACE_TRACKING_FILE)}"
                response = workspace.process.exec(cat_cmd)
                if response.exit_code == 0 and response.result:
                    data = json.loads(response.result)
                    logger.debug(f"Read workspace tracking file using Daytona exec")
                    return data.get('workspace_id'), data.get('created_at')
        except Exception as e:
            logger.warning(f"Failed to use Daytona exec to read workspace tracking: {e}")
    
    # Fallback to standard local file operations
    try:
        if os.path.exists(WORKSPACE_TRACKING_FILE):
        ...
        logger.error(f"Failed to read local workspace tracking file: {e}")
    
    return None, None

async def set_active_workspace(workspace_id: str, workspace: Optional[Workspace] = None):
    """
    ...
    # Use Daytona's command execution if a workspace is provided
    if workspace:
        try:
            # Use a heredoc to safely write the JSON content to the file
            # This avoids issues with shell quoting
            heredoc_cmd = f"""
cat <<'EOF' > {shlex.quote(WORKSPACE_TRACKING_FILE)}
{json_data}
EOF
"""
            response = workspace.process.exec(heredoc_cmd)
            if response.exit_code == 0:
                logger.debug(f"Workspace tracking file updated using Daytona exec")
                return
            else:
                raise CommandExecutionError(f"Failed to write tracking file: {response.result}")
        except Exception as e:
            logger.warning(f"Failed to use Daytona exec for workspace tracking: {e}")

    # Fallback to standard local file operations if remote command fails
    try:
        logger.debug("Falling back to local file for workspace tracking.")
        tracking_dir = os.path.dirname(WORKSPACE_TRACKING_FILE)
        os.makedirs(tracking_dir, exist_ok=True)
        with open(WORKSPACE_TRACKING_FILE, 'w') as f:
            json.dump(data, f)
    except Exception as e:
        logger.error(f"Failed to update local workspace tracking file: {e}")

async def clear_active_workspace(workspace: Optional[Workspace] = None):
    """
    ...
    if workspace:
        try:
            # Use rm -f to safely remove the file, ignoring errors if it doesn't exist
            rm_cmd = f"rm -f {shlex.quote(WORKSPACE_TRACKING_FILE)}"
            workspace.process.exec(rm_cmd)
            logger.debug(f"Workspace tracking file removed using Daytona exec")
            return
        except Exception as e:
            logger.warning(f"Failed to use Daytona exec to clear workspace tracking: {e}")
    ...
    try:
        ...
        logger.error(f"Failed to remove local workspace tracking file: {e}")
                ...
                if hasattr(workspace, 'created_at'):
                    ...
                    try:
                        ...
                        if age_seconds > max_age_seconds:
                            ...
                            daytona_instance.delete(workspace)
                            cleaned_count += 1
                        else:
